...
import { ProofType, toBN, } from './types';
import { findPoolConfigPda, findMerkleTreePda, findAssetVaultPda, findVerificationKeyPda, findSpentNullifierPda, findRelayerRegistryPda, findComplianceConfigPda, findPendingBufferPda, computeAssetId, PROGRAM_ID, } from './pda';
function findCommitmentIndexPda(programId, poolConfig, commitment) {
    return PublicKey.findProgramAddressSync([Buffer.from('commitment'), poolConfig.toBuffer(), Buffer.from(commitment)], programId);
}
...
export class WhiteProtocolClient {
    ...
    async registerAsset(poolConfig, mint) {
        ...
        const [vaultTokenAccount] = PublicKey.findProgramAddressSync([Buffer.from('vault_token'), assetVault.toBuffer()], this.programId);
        return await this.program.methods
    ...
    async deposit(poolConfig, mint, amount, commitment, proofData, encryptedNote) {
        ...
        if (ataMissing) {
            ...
            const createAtaIx = createAssociatedTokenAccountInstruction(depositor, userTokenAccount, depositor, mint);
        ...
        const tx = await this.program.methods
            .depositMasp(toBN(amount), Array.from(commitment), Array.from(assetId), Buffer.from(proofData), encryptedNote || null)
            .accounts({
            depositor,
            poolConfig,
    ...
    async withdraw(poolConfig, mint, recipient, amount, merkleRoot, nullifierHash, proofData, relayerFee) {
        ...
        if (!recipientAccountInfo) {
            ...
            const createAtaIx = createAssociatedTokenAccountInstruction(relayer, recipientTokenAccount, recipient, mint);
            preInstructions.push(createAtaIx);
        }
        ...
        if (!relayerAccountInfo) {
            const { createAssociatedTokenAccountInstruction } = await import("@solana/spl-token");
            const createRelayerAtaIx = createAssociatedTokenAccountInstruction(relayer, relayerTokenAccount, relayer, mint);
            preInstructions.push(createRelayerAtaIx);
        }
        const tx = await this.program.methods
            .withdrawMasp(Buffer.from(proofData), Array.from(merkleRoot), Array.from(nullifierHash), recipient, toBN(amount), Array.from(assetId), toBN(relayerFee ?? 0n))
            .accounts({
            relayer,
            poolConfig,
            merkleTree,
     ...
     */
    async withdrawStealth(poolConfig, mint, recipient, amount, merkleRoot, nullifierHash, proofData, ephemeralPubkey, relayerFee) {
        const relayer = this.authority;
        const assetId = computeAssetId(mint);
        const [merkleTree] = findMerkleTreePda(this.programId, poolConfig);
        const [assetVault] = findAssetVaultPda(this.programId, poolConfig, assetId);
        ...
        if (!recipientAccountInfo) {
            const { createAssociatedTokenAccountInstruction } = await import("@solana/spl-token");
            preInstructions.push(createAssociatedTokenAccountInstruction(relayer, recipientTokenAccount, recipient, mint));
        }
        const relayerAccountInfo = await connection.getAccountInfo(relayerTokenAccount);
        if (!relayerAccountInfo) {
            const { createAssociatedTokenAccountInstruction } = await import("@solana/spl-token");
            preInstructions.push(createAssociatedTokenAccountInstruction(relayer, relayerTokenAccount, relayer, mint));
        }
        const tx = await this.program.methods
            .withdrawMaspStealth(Buffer.from(proofData), Array.from(merkleRoot), Array.from(nullifierHash), recipient, toBN(amount), Array.from(assetId), toBN(relayerFee ?? 0n), Array.from(ephemeralPubkey))
            .accounts({
            relayer,
            poolConfig,
            merkleTree,
    ...
    async withdrawV2(poolConfig, mint, recipient, amount, merkleRoot, nullifierHash0, nullifierHash1, changeCommitment, proofData, relayerFee) {
        ...
        const tx = await this.program.methods
            .withdrawV2(Buffer.from(proofData), Array.from(merkleRoot), Array.from(assetId), Array.from(nullifierHash0), Array.from(nullifierHash1), Array.from(changeCommitment), recipient, toBN(amount), toBN(relayerFee ?? 0n))
            .accounts({
            relayer,
            poolConfig,
            merkleTree,
    ...
    async depositYieldSol(params) {
        ...
        if (!payer)
            throw new Error("Provider missing publicKey");
        ...
        throw new Error("depositYieldSol: Proof generation wiring not yet implemented. " +
    ...
    async withdrawYieldV2(params) {
        ...
        throw new Error("withdrawYieldV2: Proof generation and relayer submission not yet implemented. " +
 ...
 */
export function createWhiteProtocolClient(provider, idl, programId) {
