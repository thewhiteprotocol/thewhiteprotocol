...
describe("Deposit & Withdraw Integration", () => {
  ...
  before("Initialize Pool & Setup", async () => {
    ...
    if (!vkInfo) {
      ...
      const vkJson = JSON.parse(fs.readFileSync("../../circuits/deposit/build/deposit_vk.json", "utf-8"));
  ...
  it("Deposits wSOL into shielded pool", async function() {
    ...
    const wasmPath = "../../circuits/deposit/build/deposit_js/deposit.wasm";
    const zkeyPath = "../../circuits/deposit/build/deposit.zkey";
