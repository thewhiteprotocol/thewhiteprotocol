/**
 * E2E Test 3: Full Withdrawal (Larger Amount)
 * 
 * Full flow: Clear pending → Deposit wSOL (0.2) → Settle → Full Withdraw
 * Note: Partial withdrawals with change outputs require joinsplit/withdraw_v2
 */
...
async function main(): Promise<TestResult> {
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  E2E TEST 3: Full Withdrawal (Larger Amount)');
  console.log('═══════════════════════════════════════════════════════════════\n');
  ...
  try {
    ...
    const depositAmount = BigInt(0.2 * LAMPORTS_PER_SOL);
    ...
    // ==========================================
    // STEP 3: Withdraw (full amount - partial withdraw needs joinsplit)
    // ==========================================
    console.log('\n💸 STEP 3: Withdrawal\n');
    ...
    const withdrawAmount = depositAmount; // Full withdrawal
    
    console.log('Withdraw amount:', Number(withdrawAmount) / LAMPORTS_PER_SOL, 'SOL');
    
    // Build merkle proof for index 0 - use zeros as siblings
    const proofZeros: bigint[] = [];
    for (let i = 0; i < 20; i++) proofZeros.push(BigInt('0x' + Buffer.from(updatedTree.zeros[i]).toString('hex')));
    const merkleProof = proofZeros;
    ...
    const withdrawResult = await prover.generateWithdrawProof({
      ...
      recipient: authority.publicKey, amount: withdrawAmount, relayer: authority.publicKey,
      relayerFee: BigInt(0), publicDataHash: BigInt(0), secret: secret, nullifier: nullifier,
      leafIndex: startIndex, merkleProof: { pathElements: merkleProof, pathIndices: Array(20).fill(0), leaf: commitment, root: currentRoot, leafIndex: startIndex },
    });
    ...
    const withdrawTx = await (program.methods as any)
      ...
      .accountsStrict({
        relayer: authority.publicKey, poolConfig: POOL_CONFIG, merkleTree: MERKLE_TREE,
        vkAccount: withdrawVk, assetVault: assetVault, vaultTokenAccount: vaultTokenAccount,
        recipientTokenAccount: recipientTokenAccount, relayerTokenAccount: recipientTokenAccount,
        spentNullifier: spentNullifier, relayerRegistry: relayerRegistry, relayerNode: null,
        yieldRegistry: null, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SystemProgram.programId,
      })
      .preInstructions(withdrawPreInstructions)
      .rpc();
    
    console.log('✅ Partial withdrawal:', withdrawTx);
