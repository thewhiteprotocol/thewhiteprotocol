...
pub fn g1_add(a: &[u8; 64], b: &[u8; 64]) -> Result<[u8; 64]> {
    // Input: 128 bytes = G1(a) (64) + G1(b) (64)
...
#[cfg(not(any(target_os = "solana", target_arch = "bpf")))]
pub fn g1_add(a: &[u8; 64], b: &[u8; 64]) -> Result<[u8; 64]> {
    let a_point = bytes_to_g1(a).ok_or(WhiteProtocolError::CryptographyError)?;
    let b_point = bytes_to_g1(b).ok_or(WhiteProtocolError::CryptographyError)?;
...
#[cfg(not(any(target_os = "solana", target_arch = "bpf")))]
pub fn g1_mul(point: &[u8; 64], scalar: &[u8; 32]) -> Result<[u8; 64]> {
    let p = bytes_to_g1(point).ok_or(WhiteProtocolError::CryptographyError)?;
    let s = bytes_to_fr(scalar).ok_or(WhiteProtocolError::CryptographyError)?;
...
pub fn pairing_check_4(pairs: &[[u8; 192]; 4]) -> Result<bool> {
    ...
    for pair in pairs.iter() {
        ...
        let g1 = bytes_to_g1(&g1_bytes).ok_or(WhiteProtocolError::CryptographyError)?;
        let g2 = bytes_to_g2(&g2_bytes).ok_or(WhiteProtocolError::CryptographyError)?;
