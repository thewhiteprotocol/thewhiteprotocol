...
#[derive(Clone, Debug, PartialEq)]
pub struct BridgeMessageValidationError {
...
pub enum BridgeMessageError {
    ...
    InvalidMessageType,
    #[msg("Domain cannot be zero")]
    ZeroDomain,
    #[msg("Source and destination domains must differ")]
    SameDomain,
    #[msg("Amount must be greater than zero")]
    ZeroAmount,
    #[msg("Deadline cannot be zero")]
    ZeroDeadline,
    #[msg("Canonical asset ID cannot be zero")]
    ZeroCanonicalAssetId,
    #[msg("Destination commitment cannot be zero for BridgeOut")]
    ZeroDestinationCommitment,
    #[msg("Source nullifier hash cannot be zero for BridgeOut")]
    ZeroSourceNullifierHash,
    #[msg("Source finality block must be >= source block number")]
    InvalidFinalityBlock,
    #[msg("Encoded length mismatch")]
...
pub fn encode_bridge_message_v1(msg: &BridgeMessageV1) -> Result<Vec<u8>> {
    ...
    if out.len() != BRIDGE_MESSAGE_ENCODED_LENGTH {
        return Err(BridgeMessageError::EncodingLengthMismatch.into());
    }
...
pub fn hash_encoded_bridge_message_v1(encoded: &[u8]) -> Result<[u8; 32]> {
    require!(
        encoded.len() == BRIDGE_MESSAGE_ENCODED_LENGTH,
        BridgeMessageError::EncodingLengthMismatch
    );
    let mut combined = Vec::with_capacity(BRIDGE_MESSAGE_DOMAIN_SEPARATOR.len() + encoded.len());
    combined.extend_from_slice(BRIDGE_MESSAGE_DOMAIN_SEPARATOR);
    combined.extend_from_slice(encoded);
...
pub fn validate_bridge_message_v1(msg: &BridgeMessageV1) -> Result<()> {
    if msg.protocol_version != PROTOCOL_VERSION {
        return Err(BridgeMessageError::InvalidProtocolVersion.into());
    }

    if msg.message_type != MESSAGE_TYPE_BRIDGE_OUT && msg.message_type != MESSAGE_TYPE_BRIDGE_MINT {
        return Err(BridgeMessageError::InvalidMessageType.into());
    }

    if msg.source_domain == 0 {
        return Err(BridgeMessageError::ZeroDomain.into());
    }
    ...
    if msg.source_domain == msg.destination_domain {
        return Err(BridgeMessageError::SameDomain.into());
    }

    if msg.amount == 0 {
        return Err(BridgeMessageError::ZeroAmount.into());
    }

    if msg.deadline == 0 {
        return Err(BridgeMessageError::ZeroDeadline.into());
    }

    if msg.canonical_asset_id == [0u8; 32] {
        return Err(BridgeMessageError::ZeroCanonicalAssetId.into());
    }

    if msg.message_type == MESSAGE_TYPE_BRIDGE_OUT && msg.destination_commitment == [0u8; 32] {
        return Err(BridgeMessageError::ZeroDestinationCommitment.into());
    }

    if msg.message_type == MESSAGE_TYPE_BRIDGE_OUT && msg.source_nullifier_hash == [0u8; 32] {
        return Err(BridgeMessageError::ZeroSourceNullifierHash.into());
    }

    if msg.source_finality_block < msg.source_block_number {
        return Err(BridgeMessageError::InvalidFinalityBlock.into());
    }

    Ok(())
}

/// Non-throwing validation. Returns a list of errors (empty if valid).
pub fn validate_bridge_message_v1_list(msg: &BridgeMessageV1) -> Vec<BridgeMessageValidationError> {
    let mut errors = Vec::new();

    if msg.protocol_version != PROTOCOL_VERSION {
        errors.push(BridgeMessageValidationError {
    ...
    if msg.message_type != MESSAGE_TYPE_BRIDGE_OUT && msg.message_type != MESSAGE_TYPE_BRIDGE_MINT {
        errors.push(BridgeMessageValidationError {
            ...
            message: "message_type must be 1 (BridgeOut) or 2 (BridgeMint)",
        });
    }
    ...
    if msg.source_domain == 0 {
        errors.push(BridgeMessageValidationError {
            field: "source_domain",
            code: "ZERO_DOMAIN",
            message: "source_domain cannot be 0",
        });
    }
    ...
    if msg.destination_domain == 0 {
        errors.push(BridgeMessageValidationError {
            ...
            code: "ZERO_DOMAIN",
            message: "destination_domain cannot be 0",
        });
    }
    ...
    if msg.source_domain == msg.destination_domain {
        errors.push(BridgeMessageValidationError {
            field: "destination_domain",
            code: "SAME_DOMAIN",
            message: "source_domain and destination_domain must be different",
        });
    }
    ...
    if msg.canonical_asset_id == [0u8; 32] {
        errors.push(BridgeMessageValidationError {
            ...
            code: "ZERO_ASSET",
            message: "canonical_asset_id cannot be zero",
        });
    }

    if msg.message_type == MESSAGE_TYPE_BRIDGE_OUT && msg.destination_commitment == [0u8; 32] {
        errors.push(BridgeMessageValidationError {
            field: "destination_commitment",
            code: "ZERO_COMMITMENT",
            message: "destination_commitment cannot be zero for BridgeOut",
        });
    }

    if msg.message_type == MESSAGE_TYPE_BRIDGE_OUT && msg.source_nullifier_hash == [0u8; 32] {
        errors.push(BridgeMessageValidationError {
            field: "source_nullifier_hash",
            code: "ZERO_NULLIFIER",
            message: "source_nullifier_hash cannot be zero for BridgeOut",
        });
    }

    if msg.source_finality_block < msg.source_block_number {
        errors.push(BridgeMessageValidationError {
            field: "source_finality_block",
            code: "INVALID_FINALITY",
            message: "source_finality_block must be >= source_block_number",
        });
    }
...
mod tests {
    ...
    fn make_valid_message() -> BridgeMessageV1 {
        BridgeMessageV1 {
            ...
            source_nullifier_hash: hex_to_bytes32("1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef"),
            destination_commitment: hex_to_bytes32("fedcba0987654321fedcba0987654321fedcba0987654321fedcba0987654321"),
            source_root: hex_to_bytes32("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"),
            source_leaf_index: 7,
            source_tx_hash: hex_to_bytes32("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"),
    ...
    #[test]
    fn test_rejects_invalid_protocol_version() {
        let mut msg = make_valid_message();
        msg.protocol_version = 2;
        assert!(encode_bridge_message_v1(&msg).is_err());
    }
    ...
    #[test]
    fn test_rejects_zero_source_domain() {
        let mut msg = make_valid_message();
        msg.source_domain = 0;
        assert!(encode_bridge_message_v1(&msg).is_err());
    }
    ...
    #[test]
    fn test_rejects_zero_destination_domain() {
        let mut msg = make_valid_message();
        msg.destination_domain = 0;
        assert!(encode_bridge_message_v1(&msg).is_err());
    }

    #[test]
    fn test_rejects_same_domain() {
        let mut msg = make_valid_message();
        msg.destination_domain = msg.source_domain;
        assert!(encode_bridge_message_v1(&msg).is_err());
    }
    ...
    #[test]
    fn test_rejects_zero_canonical_asset_id() {
        let mut msg = make_valid_message();
        msg.canonical_asset_id = [0u8; 32];
        assert!(encode_bridge_message_v1(&msg).is_err());
    }

    #[test]
    fn test_rejects_zero_destination_commitment_for_bridge_out() {
        let mut msg = make_valid_message();
        msg.destination_commitment = [0u8; 32];
        assert!(encode_bridge_message_v1(&msg).is_err());
    }

    #[test]
    fn test_rejects_zero_nullifier_for_bridge_out() {
        let mut msg = make_valid_message();
        msg.source_nullifier_hash = [0u8; 32];
        assert!(encode_bridge_message_v1(&msg).is_err());
    }

    #[test]
    fn test_allows_bridge_mint_with_zero_commitment() {
        let mut msg = make_valid_message();
        msg.message_type = MESSAGE_TYPE_BRIDGE_MINT;
        msg.destination_commitment = [0u8; 32];
        msg.source_nullifier_hash = [0u8; 32];
        let encoded = encode_bridge_message_v1(&msg).unwrap();
        assert_eq!(encoded.len(), BRIDGE_MESSAGE_ENCODED_LENGTH);
    ...
    #[test]
    fn test_rejects_invalid_finality_block() {
        let mut msg = make_valid_message();
        msg.source_finality_block = msg.source_block_number - 1;
        assert!(encode_bridge_message_v1(&msg).is_err());
    }
    ...
    fn test_vector2_bnb_to_polygon_bridge_out() {
        let msg = BridgeMessageV1 {
            ...
            source_nullifier_hash: hex_to_bytes32("deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef"),
            destination_commitment: hex_to_bytes32("cafebabecafebabecafebabecafebabecafebabecafebabecafebabecafebabe"),
            source_root: hex_to_bytes32("1111111111111111111111111111111111111111111111111111111111111111"),
            source_leaf_index: 42,
            source_tx_hash: hex_to_bytes32("2222222222222222222222222222222222222222222222222222222222222222"),
    ...
    fn test_vector3_solana_to_base_bridge_out() {
        let msg = BridgeMessageV1 {
            ...
            source_nullifier_hash: hex_to_bytes32("abcdefabcdefabcdefabcdefabcdefabcdefabcdefabcdefabcdefabcdefabcd"),
            destination_commitment: hex_to_bytes32("5555555555555555555555555555555555555555555555555555555555555555"),
            source_root: hex_to_bytes32("6666666666666666666666666666666666666666666666666666666666666666"),
            source_leaf_index: 0,
            source_tx_hash: hex_to_bytes32("7777777777777777777777777777777777777777777777777777777777777777"),
            source_block_number: 150000000,
            source_finality_block: 150000032,
            nonce: 3,
            deadline: 1780000000,
            relayer_fee: 2_500_000_000_000_000u128,
            recipient_stealth_metadata_hash: hex_to_bytes32("8888888888888888888888888888888888888888888888888888888888888888"),
            memo_hash: hex_to_bytes32("9999999999999999999999999999999999999999999999999999999999999999"),
            reserved0: [0u8; 32],
            reserved1: [0u8; 32],
        };
        let encoded = encode_bridge_message_v1(&msg).unwrap();
    ...
    fn test_vector4_eth_to_base_bridge_mint() {
        let msg = BridgeMessageV1 {
            ...
            source_nullifier_hash: hex_to_bytes32("0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20"),
            destination_commitment: hex_to_bytes32("202122232425262728292a2b2c2d2e2f303132333435363738393a3b3c3d3e3f"),
            source_root: hex_to_bytes32("404142434445464748494a4b4c4d4e4f505152535455565758595a5b5c5d5e5f"),
            source_leaf_index: 100,
            source_tx_hash: hex_to_bytes32("606162636465666768696a6b6c6d6e6f707172737475767778797a7b7c7d7e7f"),
