/**
 * PR-010K Full Bridge E2E: Ethereum Sepolia -> Base Sepolia
 * Reverse direction: Ethereum source note -> bridgeOutV1 -> Base destination withdraw.
 *
 * Flow:
 * 1. Deposit native asset on Ethereum Sepolia WhiteProtocol
 * 2. Settle deposit into Ethereum Merkle tree
 * 3. Generate destination note (secret/nullifier) for Base withdraw
 ...
 * 10. Submit Base BridgeInbox.acceptBridgeMint
 * 11. Verify destination commitment insertion
 * 12. Generate Base withdraw proof for destination note
 * 13. Call WhiteProtocol.withdraw on Base
 * 14. Verify duplicate withdraw/nullifier replay rejected
 * 15. Verify duplicate bridge replay rejected
 *
 * Usage:
 *   export DEPLOYER_PRIVATE_KEY=0x...
 *   source chains/evm/.bridge-signers.env
 *   cd chains/evm && npm run test:e2e:bridge:ethereum-to-base:full
 */
...
const DEST_RPC = process.env.BASE_SEPOLIA_RPC_URL || 'https://base-sepolia-rpc.publicnode.com';
...
const sourceArtifact = JSON.parse(
  fs.readFileSync(path.join(__dirname, '../deployments/ethereum-sepolia.json'), 'utf8')
);
const destArtifact = JSON.parse(
...
async function main() {
  console.log('═══════════════════════════════════════════════════════════');
  console.log('  PR-010K FULL BRIDGE E2E');
  console.log('  Ethereum Sepolia -> Base Sepolia');
  console.log('  (Reverse direction with destination withdraw)');
  console.log('═══════════════════════════════════════════════════════════\n');
  ...
  const sourceWP = new ethers.Contract(sourceArtifact.contracts.WhiteProtocol, WHITEPROTOCOL_ABI, sourceWallet);
  const sourceOutbox = new ethers.Contract(sourceArtifact.bridgeV1.BridgeOutbox, BRIDGE_OUTBOX_ABI, sourceWallet);
  const sourceRegistry = new ethers.Contract(sourceArtifact.contracts.AssetRegistry, ASSETREGISTRY_ABI, sourceProvider);
  const destInbox = new ethers.Contract(destArtifact.bridgeV1.BridgeInbox, BRIDGE_INBOX_ABI, destWallet);
  const destWP = new ethers.Contract(destArtifact.contracts.WhiteProtocol, WHITEPROTOCOL_ABI, destWallet);

  console.log('Deployer:', wallet.address);
  console.log('Source WhiteProtocol (Ethereum):', sourceWP.address);
  console.log('Source BridgeOutbox (Ethereum):', sourceOutbox.address);
  console.log('Dest BridgeInbox (Base):', destInbox.address);
  console.log('Dest WhiteProtocol (Base):', destWP.address);

  // Verify wiring
  const wpOutbox = await sourceWP.bridgeOutbox();
  const outboxWP = await sourceOutbox.whiteProtocol();
  console.log('WhiteProtocol.bridgeOutbox:', wpOutbox);
  console.log('BridgeOutbox.whiteProtocol:', outboxWP);
  if (wpOutbox.toLowerCase() !== sourceOutbox.address.toLowerCase()) {
    throw new Error('WhiteProtocol.bridgeOutbox mismatch');
  }
  if (outboxWP.toLowerCase() !== sourceWP.address.toLowerCase()) {
    throw new Error('BridgeOutbox.whiteProtocol mismatch');
  }
  console.log('✅ Wiring verified\n');

  const SOURCE_ASSET_ID = BigInt((await sourceRegistry.getAssetId(ethers.constants.AddressZero)).toString());
  console.log('Source Asset ID:', '0x' + SOURCE_ASSET_ID.toString(16).padStart(64, '0'));

  const destRegistry = new ethers.Contract(destArtifact.contracts.AssetRegistry, ASSETREGISTRY_ABI, destProvider);
  const DEST_ASSET_ID = BigInt((await destRegistry.getAssetId(ethers.constants.AddressZero)).toString());
  console.log('Dest Asset ID:', '0x' + DEST_ASSET_ID.toString(16).padStart(64, '0'));

  const depositAmount = ethers.utils.parseEther('0.001');

  // ───────────────────────────────────────────────────────────
  // STEP 0: Settle any existing pending deposits on source
  // ───────────────────────────────────────────────────────────
  console.log('───────────────────────────────────────────────────────────');
  console.log('STEP 0: SETTLE EXISTING PENDING DEPOSITS');
  console.log('───────────────────────────────────────────────────────────');

  let treeState = await getTreeState(sourceWP);
  ...
  const pendingCommitments = await getPendingDeposits(sourceWP);
  ...
  if (pendingCommitments.length > 0) {
    for (let i = 0; i < pendingCommitments.length; i++) {
      ...
      const result = await settleSingleDeposit(sourceWP, pendingCommitments[i], treeState);
  ...
  // ───────────────────────────────────────────────────────────
  // STEP 0b: PROVIDE LIQUIDITY ON DESTINATION
  // ───────────────────────────────────────────────────────────
  console.log('───────────────────────────────────────────────────────────');
  console.log('STEP 0b: PROVIDE LIQUIDITY ON DESTINATION');
  console.log('───────────────────────────────────────────────────────────');

  const destWPBalanceBefore = await destProvider.getBalance(destWP.address);
  console.log('Dest WhiteProtocol balance before:', ethers.utils.formatEther(destWPBalanceBefore), 'ETH');

  if (destWPBalanceBefore.lt(depositAmount)) {
    console.log('Sending ETH to destination WhiteProtocol for withdraw liquidity...');
    const sendTx = await destWallet.sendTransaction({
      to: destWP.address,
      value: depositAmount,
    });
    await sendTx.wait();
    const destWPBalanceAfter = await destProvider.getBalance(destWP.address);
    console.log('Dest WhiteProtocol balance after:', ethers.utils.formatEther(destWPBalanceAfter), 'ETH');
  }
  ...
  const testCommitment = await computeCommitment(testSecret, testNullifier, BigInt(depositAmount.toString()), SOURCE_ASSET_ID);

  // Destination note for Base withdraw
  const destSecret = randomBigInt(31);
  const destNullifier = randomBigInt(31);
  const destCommitment = await computeCommitment(destSecret, destNullifier, BigInt(depositAmount.toString()), DEST_ASSET_ID);
  ...
  console.log('Destination note (for Base withdraw):');
  ...
  const depositInput = {
    ...
    asset_id: SOURCE_ASSET_ID.toString(),
  ...
  const depositTx = await sourceWP.deposit(
    depositProofBytes,
    testCommitment,
    depositAmount,
  ...
  treeState = await getTreeState(sourceWP);
  const settleResult = await settleSingleDeposit(sourceWP, testCommitment, treeState);
  ...
  // Get next nonce from source outbox for destination domain
  const nextNonce = await sourceOutbox.outboundNonce(33554434);
  ...
  const message = {
    ...
    canonicalAssetId: '0x' + SOURCE_ASSET_ID.toString(16).padStart(64, '0'),
    sourceLocalAssetId: '0x' + SOURCE_ASSET_ID.toString(16).padStart(64, '0'),
    destinationLocalAssetId: destArtifact.bridgeV1.canonicalAssetId,
    amount: bridgeAmount,
  ...
  console.log('Amount:', ethers.utils.formatEther(message.amount.toString()), 'ETH');
  ...
  const bridgeOutboxAddress = sourceOutbox.address;
  ...
  // ───────────────────────────────────────────────────────────
  console.log('───────────────────────────────────────────────────────────');
  console.log('STEP E: CALL bridgeOutV1 (Ethereum Sepolia)');
  console.log('───────────────────────────────────────────────────────────');

  const bridgeOutTx = await sourceWP.bridgeOutV1(
    withdrawProofBytes,
    message,
    ethers.constants.AddressZero
  );
  ...
  const isSpent = await sourceWP.isSpent(nullifierHash);
  if (!isSpent) {
  ...
  console.log('BridgeOutbox.whiteProtocol:', await sourceOutbox.whiteProtocol());
  ...
  await sourceProvider.waitForTransaction(bridgeOutTx.hash, finalityBlocks);
  ...
  // ───────────────────────────────────────────────────────────
  // STEP I: Record pre-state on destination
  // ───────────────────────────────────────────────────────────
  console.log('───────────────────────────────────────────────────────────');
  console.log('STEP I: RECORD PRE-STATE (Base Sepolia)');
  console.log('───────────────────────────────────────────────────────────');

  const preRoot = await destWP.getLastRoot();
  const preLeafIndex = await destWP.nextLeafIndex();
  console.log('Pre-state root:', preRoot.toHexString());
  console.log('Pre-state nextLeafIndex:', preLeafIndex.toString());

  // ───────────────────────────────────────────────────────────
  // STEP J: Submit acceptBridgeMint on destination
  // ───────────────────────────────────────────────────────────
  console.log('\n───────────────────────────────────────────────────────────');
  console.log('STEP J: SUBMIT ACCEPT BRIDGE MINT (Base Sepolia)');
  console.log('───────────────────────────────────────────────────────────');
  ...
  try {
    ...
    if (e.code === 'UNPREDICTABLE_GAS_LIMIT') {
      console.log('Gas estimation failed, retrying with manual gas limit (1,500,000)...');
  ...
  console.log('  amount:', ethers.utils.formatEther(mintEvent.args.amount.toString()), 'ETH');
  ...
  const postRoot = await destWP.getLastRoot();
  const postLeafIndex = await destWP.nextLeafIndex();
  ...
  const isConsumedBefore = await destInbox.isMessageConsumed(messageHash);
  console.log('Message already consumed:', isConsumedBefore);

  try {
    await destInbox.acceptBridgeMint(message, thresholdSigs, 1);
  ...
  // ───────────────────────────────────────────────────────────
  // STEP M: DESTINATION WITHDRAW (Base Sepolia)
  // ───────────────────────────────────────────────────────────
  console.log('───────────────────────────────────────────────────────────');
  console.log('STEP M: DESTINATION WITHDRAW (Base Sepolia)');
  console.log('───────────────────────────────────────────────────────────');

  const destLeafIndex = Number(preLeafIndex.toString());
  console.log('Destination leaf index:', destLeafIndex);

  const destTreeState = await getTreeState(destWP);
  const destPath = computePath(destLeafIndex, destTreeState.filledSubtrees, destTreeState.zeros);

  const destVerifyRoot = await computeRootFromPath(destCommitment, destPath);
  verifyRootMatch(destVerifyRoot, destTreeState.currentRoot, 'Destination withdrawal path');
  ...
  const destWithdrawInput = {
    ...
    amount: bridgeAmount.toString(),
    asset_id: DEST_ASSET_ID.toString(),
    ...
    recipient: BigInt(destWallet.address).toString(),
    relayer: '0',
    relayer_fee: '0',
    public_data_hash: '0',
  };
  ...
  const expectedDestPubSignals = [
    ...
    DEST_ASSET_ID.toString(),
    BigInt(destWallet.address).toString(),
    bridgeAmount.toString(),
    '0',
    '0',
    '0',
  ...
  const destBalanceBefore = await destProvider.getBalance(destWallet.address);
  console.log('Dest wallet balance before withdraw:', ethers.utils.formatEther(destBalanceBefore), 'ETH');

  const destWithdrawTx = await destWP.withdraw(
  ...
  const destBalanceAfter = await destProvider.getBalance(destWallet.address);
  console.log('Dest wallet balance after withdraw:', ethers.utils.formatEther(destBalanceAfter), 'ETH');

  const destNullifierSpent = await destWP.isSpent(destNullifierHash);
  ...
  // ───────────────────────────────────────────────────────────
  console.log('═══════════════════════════════════════════════════════════');
  console.log('  PR-010K FULL BRIDGE E2E RESULTS');
  console.log('═══════════════════════════════════════════════════════════');
  console.log('Route:                Ethereum Sepolia -> Base Sepolia');
  console.log('Source WhiteProtocol: ', sourceWP.address);
  console.log('Source BridgeOutbox:  ', sourceOutbox.address);
  console.log('Dest BridgeInbox:     ', destInbox.address);
  console.log('Dest WhiteProtocol:   ', destWP.address);
  console.log('Message hash:         ', messageHash);
  ...
  console.log('═══════════════════════════════════════════════════════════');
  console.log('ALL PR-010K E2E TESTS PASSED ✓');

  // Explicit cleanup
  sourceProvider.removeAllListeners();
  destProvider.removeAllListeners();
  process.exit(0);
}
