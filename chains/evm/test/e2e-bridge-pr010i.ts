/**
 * Canonical Base Sepolia -> Ethereum Sepolia Full Private Bridge E2E
 ...
 * 3. Generate destination note (secret/nullifier) for Ethereum withdraw
 ...
 * 10. Submit Ethereum BridgeInbox.acceptBridgeMint
 * 11. Verify destination commitment insertion
 * 12. Generate Ethereum withdraw proof for destination note
 * 13. Call WhiteProtocol.withdraw on Ethereum (using signer, not read-only provider)
...
async function main() {
  ...
  const ethInbox = new ethers.Contract(ethArtifact.bridgeV1.BridgeInbox, BRIDGE_INBOX_ABI, ethWallet);
  ...
  console.log('Eth BridgeInbox:', ethInbox.address);
  console.log('Eth WhiteProtocol:', ethWP.address);
  ...
  const ethRegistry = new ethers.Contract(ethArtifact.contracts.AssetRegistry, ASSETREGISTRY_ABI, ethProvider);
  const ETH_ASSET_ID = BigInt((await ethRegistry.getAssetId(ethers.constants.AddressZero)).toString());
  console.log('Eth Asset ID:', '0x' + ETH_ASSET_ID.toString(16).padStart(64, '0'));
  ...
  // ───────────────────────────────────────────────────────────
  console.log('───────────────────────────────────────────────────────────');
  console.log('STEP 0b: PROVIDE LIQUIDITY ON ETHEREUM');
  console.log('───────────────────────────────────────────────────────────');

  const ethWPBalanceBefore = await ethProvider.getBalance(ethWP.address);
  console.log('Eth WhiteProtocol balance before:', ethers.utils.formatEther(ethWPBalanceBefore), 'ETH');

  if (ethWPBalanceBefore.lt(depositAmount)) {
    console.log('Sending ETH to Ethereum WhiteProtocol for withdraw liquidity...');
    const sendTx = await ethWallet.sendTransaction({
      to: ethWP.address,
      value: depositAmount,
    });
    await sendTx.wait();
    const ethWPBalanceAfter = await ethProvider.getBalance(ethWP.address);
    console.log('Eth WhiteProtocol balance after:', ethers.utils.formatEther(ethWPBalanceAfter), 'ETH');
  }
  ...
  const destCommitment = await computeCommitment(destSecret, destNullifier, BigInt(depositAmount.toString()), ETH_ASSET_ID);
  ...
  const nextNonce = await baseOutbox.outboundNonce(33554435);
  ...
  const message = {
    ...
    destinationLocalAssetId: ethArtifact.bridgeV1.canonicalAssetId,
    amount: bridgeAmount,
  ...
  // ───────────────────────────────────────────────────────────
  console.log('───────────────────────────────────────────────────────────');
  console.log('STEP I: RECORD PRE-STATE (Ethereum Sepolia)');
  console.log('───────────────────────────────────────────────────────────');

  const preRoot = await ethWP.getLastRoot();
  const preLeafIndex = await ethWP.nextLeafIndex();
  ...
  // ───────────────────────────────────────────────────────────
  console.log('\n───────────────────────────────────────────────────────────');
  console.log('STEP J: SUBMIT ACCEPT BRIDGE MINT (Ethereum Sepolia)');
  console.log('───────────────────────────────────────────────────────────');

  const acceptTx = await ethInbox.acceptBridgeMint(message, thresholdSigs, 1);
  ...
  const postRoot = await ethWP.getLastRoot();
  const postLeafIndex = await ethWP.nextLeafIndex();
  ...
  const isConsumedBefore = await ethInbox.isMessageConsumed(messageHash);
  console.log('Message already consumed:', isConsumedBefore);

  try {
    await ethInbox.acceptBridgeMint(message, thresholdSigs, 1);
  ...
  // ───────────────────────────────────────────────────────────
  console.log('───────────────────────────────────────────────────────────');
  console.log('STEP M: DESTINATION WITHDRAW (Ethereum Sepolia)');
  console.log('───────────────────────────────────────────────────────────');

  const destLeafIndex = Number(preLeafIndex.toString());
  console.log('Destination leaf index:', destLeafIndex);

  const ethTreeState = await getTreeState(ethWP);
  ...
  const destWithdrawInput = {
    ...
    recipient: BigInt(ethWallet.address).toString(),
    relayer: '0',
    relayer_fee: '0',
    public_data_hash: '0',
  };
  ...
  const expectedDestPubSignals = [
    ...
    BigInt(ethWallet.address).toString(),
    bridgeAmount.toString(),
    '0',
    '0',
    '0',
  ...
  const ethBalanceBefore = await ethProvider.getBalance(ethWallet.address);
  console.log('Eth wallet balance before withdraw:', ethers.utils.formatEther(ethBalanceBefore), 'ETH');

  const destWithdrawTx = await ethWP.withdraw(
  ...
  const ethBalanceAfter = await ethProvider.getBalance(ethWallet.address);
  console.log('Eth wallet balance after withdraw:', ethers.utils.formatEther(ethBalanceAfter), 'ETH');

  // Verify nullifier is spent
  const destNullifierSpent = await ethWP.isSpent(destNullifierHash);
