/**
 * Canonical Base Sepolia -> BSC Testnet Full Private Bridge E2E
 * Covers PR-010I + PR-010J variant for BSC Testnet, including destination withdrawal.
 *
 * Flow:
 * 1. Deposit native asset on Base Sepolia WhiteProtocol
 * 2. Settle deposit into Merkle tree
 * 3. Generate destination note (secret/nullifier) for BSC Testnet withdraw
 ...
 * 10. Submit BSC Testnet BridgeInbox.acceptBridgeMint
 * 11. Verify destination commitment insertion
 * 12. Generate BSC Testnet withdraw proof for destination note
 * 13. Call WhiteProtocol.withdraw on BSC Testnet (using signer, not read-only provider)
 ...
 *   cd chains/evm && npm run test:e2e:bridge:base-to-ethereum:full
 */
...
const BNB_RPC = 'https://bsc-testnet-rpc.publicnode.com';
...
const bnbArtifact = JSON.parse(
  fs.readFileSync(path.join(__dirname, '../deployments/bsc-testnet.json'), 'utf8')
);
...
async function main() {
  console.log('═══════════════════════════════════════════════════════════');
  console.log('  PR-010I + PR-010J FULL BRIDGE E2E (BASE -> BSC)');
  console.log('  Base Sepolia -> BSC Testnet');
  ...
  const bnbInbox = new ethers.Contract(bnbArtifact.bridgeV1.BridgeInbox, BRIDGE_INBOX_ABI, bnbWallet);
  const bnbWP = new ethers.Contract(bnbArtifact.contracts.WhiteProtocol, WHITEPROTOCOL_ABI, bnbWallet);

  console.log('Deployer:', wallet.address);
  console.log('Base WhiteProtocol:', baseWP.address);
  console.log('Base BridgeOutbox:', baseOutbox.address);
  console.log('Bnb BridgeInbox:', bnbInbox.address);
  console.log('Bnb WhiteProtocol:', bnbWP.address);
  ...
  const bnbRegistry = new ethers.Contract(bnbArtifact.contracts.AssetRegistry, ASSETREGISTRY_ABI, bnbProvider);
  const BNB_ASSET_ID = BigInt((await bnbRegistry.getAssetId(ethers.constants.AddressZero)).toString());
  console.log('Bnb Asset ID:', '0x' + BNB_ASSET_ID.toString(16).padStart(64, '0'));
  ...
  // ───────────────────────────────────────────────────────────
  console.log('───────────────────────────────────────────────────────────');
  console.log('STEP 0b: PROVIDE LIQUIDITY ON BSC TESTNET');
  console.log('───────────────────────────────────────────────────────────');

  const bnbWPBalanceBefore = await bnbProvider.getBalance(bnbWP.address);
  console.log('Bnb WhiteProtocol balance before:', ethers.utils.formatEther(bnbWPBalanceBefore), 'BNB');

  if (bnbWPBalanceBefore.lt(depositAmount)) {
    console.log('Sending BNB to BSC Testnet WhiteProtocol for withdraw liquidity...');
    const sendTx = await bnbWallet.sendTransaction({
      to: bnbWP.address,
      value: depositAmount,
    });
    await sendTx.wait();
    const bnbWPBalanceAfter = await bnbProvider.getBalance(bnbWP.address);
    console.log('Bnb WhiteProtocol balance after:', ethers.utils.formatEther(bnbWPBalanceAfter), 'BNB');
  }
  ...
  // Destination note for Ethereum withdraw
  const destSecret = randomBigInt(31);
  const destNullifier = randomBigInt(31);
  const destCommitment = await computeCommitment(destSecret, destNullifier, BigInt(depositAmount.toString()), BNB_ASSET_ID);
  ...
  console.log('Destination note (for Eth withdraw):');
  ...
  // Get next nonce from Base outbox for Ethereum destination
  const nextNonce = await baseOutbox.outboundNonce(33554438);
  ...
  const message = {
    ...
    destinationLocalAssetId: bnbArtifact.bridgeV1.canonicalAssetId,
    amount: bridgeAmount,
  ...
  // ───────────────────────────────────────────────────────────
  console.log('───────────────────────────────────────────────────────────');
  console.log('STEP I: RECORD PRE-STATE (BSC Testnet)');
  console.log('───────────────────────────────────────────────────────────');

  const preRoot = await bnbWP.getLastRoot();
  const preLeafIndex = await bnbWP.nextLeafIndex();
  ...
  // ───────────────────────────────────────────────────────────
  console.log('\n───────────────────────────────────────────────────────────');
  console.log('STEP J: SUBMIT ACCEPT BRIDGE MINT (BSC Testnet)');
  console.log('───────────────────────────────────────────────────────────');

  const acceptTx = await bnbInbox.acceptBridgeMint(message, thresholdSigs, 1, { gasLimit: 1500000 });
  const acceptReceipt = await acceptTx.wait();
  console.log('acceptBridgeMint tx:', acceptTx.hash);

  // Post-confirmation delay for BSC RPC state lag
  await new Promise(r => setTimeout(r, 3000));
  ...
  const postRoot = await bnbWP.getLastRoot();
  const postLeafIndex = await bnbWP.nextLeafIndex();
  ...
  const isConsumedBefore = await bnbInbox.isMessageConsumed(messageHash);
  console.log('Message already consumed:', isConsumedBefore);

  try {
    await bnbInbox.acceptBridgeMint(message, thresholdSigs, 1);
  ...
  // ───────────────────────────────────────────────────────────
  // STEP M: DESTINATION WITHDRAW (Ethereum Sepolia)
  // ───────────────────────────────────────────────────────────
  console.log('───────────────────────────────────────────────────────────');
  console.log('STEP M: DESTINATION WITHDRAW (BSC Testnet)');
  console.log('───────────────────────────────────────────────────────────');

  const destLeafIndex = Number(preLeafIndex.toString());
  console.log('Destination leaf index:', destLeafIndex);

  const ethTreeState = await getTreeState(bnbWP);
  const destPath = computePath(destLeafIndex, ethTreeState.filledSubtrees, ethTreeState.zeros);

  // Verify path leads to current root
  const destVerifyRoot = await computeRootFromPath(destCommitment, destPath);
  verifyRootMatch(destVerifyRoot, ethTreeState.currentRoot, 'Destination withdrawal path');
  ...
  const destWithdrawInput = {
    ...
    amount: bridgeAmount.toString(),
    asset_id: BNB_ASSET_ID.toString(),
    leaf_index: destLeafIndex.toString(),
    merkle_root: ethTreeState.currentRoot.toString(),
    ...
    recipient: BigInt(bnbWallet.address).toString(),
    relayer: '0',
    relayer_fee: '0',
    public_data_hash: '0',
  };
  ...
  const expectedDestPubSignals = [
    ethTreeState.currentRoot.toString(),
    destNullifierHash.toString(),
    BNB_ASSET_ID.toString(),
    BigInt(bnbWallet.address).toString(),
    bridgeAmount.toString(),
    '0',
    '0',
    '0',
  ...
  const bnbBalanceBefore = await bnbProvider.getBalance(bnbWallet.address);
  console.log('Bnb wallet balance before withdraw:', ethers.utils.formatEther(bnbBalanceBefore), 'BNB');

  const destWithdrawTx = await bnbWP.withdraw(
    destWithdrawProofBytes,
    destNullifierHash,
    ethTreeState.currentRoot,
    bnbWallet.address,
    ethers.constants.AddressZero,
    bridgeAmount,
    0,
  ...
  const bnbBalanceAfter = await bnbProvider.getBalance(bnbWallet.address);
  console.log('Bnb wallet balance after withdraw:', ethers.utils.formatEther(bnbBalanceAfter), 'BNB');

  // Verify nullifier is spent
  const destNullifierSpent = await bnbWP.isSpent(destNullifierHash);
  ...
  console.log('═══════════════════════════════════════════════════════════');
  console.log('Route:                Base Sepolia -> BSC Testnet');
  console.log('Base WhiteProtocol:   ', baseWP.address);
  console.log('Base BridgeOutbox:    ', baseOutbox.address);
  console.log('Bnb BridgeInbox:      ', bnbInbox.address);
  console.log('Bnb WhiteProtocol:    ', bnbWP.address);
  ...
  bnbProvider.removeAllListeners();
  process.exit(0);
}
