...
- [x] Solana -> Base source-only runner now exports exact Base destination note-state to `BRIDGE_BASE_NOTE_STATE_BACKUP_DIR` during fresh source fixture generation, refuses `/tmp` and repo-local backup paths, and reports only the durable backup path. Local PR-013M execution is blocked by missing live RPC/signer/wallet env and absent `/data`; no Base destination transaction was submitted.
