...
- [x] Fresh Solana -> Base durable source-fixture flow prepared so the source-only command can write `/data/bridge-results/solana-to-base-source-fixture-<sourceMessageHash>.json` directly after generating a new source hash. Local execution remained pending because `/data` and hosted RPC/signer/wallet env were absent; no Base destination submit was attempted (PR-013G).
