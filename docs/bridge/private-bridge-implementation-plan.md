...
| Solana `bridge_out_v1_with_proof` | Implemented & live-tested | Real | Yes — withdraw proof + nullifier + custody lock | Medium | Complete live hosted replay after PR-012X paper-mode source support |
...
- [x] Solana Devnet -> Base Sepolia hosted paper-flow foundation added with reverse route metadata, exact 9-to-18 decimal normalization, fixture/historical Solana source replay for `bridge_out_v1_with_proof`, unsafe `init_bridge_v1_out` rejection tests, Base `acceptBridgeMint` preview generation, and no destination submission (PR-012X).
- [x] Historical Solana Devnet -> Base Sepolia PR-010Z source transaction was reconstructed into a non-secret fixture and replayed in paper mode; the event parsed and persisted, but policy correctly blocked signing because the historical deadline expired, so no Base preview or destination transaction was produced (PR-012Y).
- [x] Fresh Solana Devnet -> Base Sepolia source event generated with `bridge_out_v1_with_proof`, normalized 9-to-18 Base destination amount, non-secret fixture export, source finality satisfied, paper replay reached `paper_ready_to_submit`, two signatures were produced, and a Base `acceptBridgeMint` preview was created with no Base destination transaction submitted (PR-013A).
- [x] Guarded Solana Devnet -> Base Sepolia operator approval readiness added for the PR-013A paper message. Base read-only checks pass for destination hash `0xddcc4a5c4c4522ae983186dc8eb10f9e3ad4d2ba36f3ca31ef386d0528a62c83`, but Base `acceptBridgeMint` simulation blocks with `InvalidSigner`, so this exact message is not approved for live destination submit (PR-013B).
- [x] Solana Devnet -> Base Sepolia paper message re-signed with deployed Base signer-set keys. Recovered signers match Base signer set version `1` threshold `2`, `InvalidSigner` is resolved, and Base `acceptBridgeMint` simulation passes with no destination transaction submitted (PR-013C).
- [x] Guarded one-shot Base Sepolia submit command added for the exact approved Solana -> Base destination BridgeMint hash. The command requires exact route/hash approval, reruns Base checks and simulation before send, and blocks broad live relaying; the PR-013D live attempt fail-closed before submission because the approval-ready paper state under `/tmp/pr013a-solana-to-base-paper-state` was unavailable.
- [x] Durable Solana -> Base approval-state restore was attempted locally for PR-013E and remained blocked before submit because the hosted `/data` mount and both fixture paths were unavailable in this workspace. The public Solana source transaction is finalized, but the non-secret fixture and paper-ready state must be restored on Render before approval and guarded submit can rerun.
- [x] Read-only Solana -> Base fixture reconstruction command added for finalized source transactions. The PR-013A fixture reconstructs with matching source and destination hashes, but replay now correctly rejects the original message with `expired_deadline`; no approval rerun or Base destination submit was attempted (PR-013F).
- [x] Fresh Solana -> Base durable paper approval completed on Render for source tx `3uTMH3jsARmS49MF7SEqgq2Tv4ahosAdK9Vz4GP7BdPNS2mSsVDhwnTAsjiNPte6M5YCSBZwCgREqFvNJy8ZbbYF`. The non-secret fixture was restored to `/data/bridge-results/solana-to-base-source-fixture-0x0fd0e20315403767f28cac97ece9b8937c984aba43bc7f575219de681abda198.json`, paper replay persisted `/data/bridge-results/solana-to-base-paper-state`, policy/finality passed, two signatures were produced, Base `acceptBridgeMint` preview was created, approval rerun returned `approval_ready`, simulation passed with gas estimate `986321`, and no Base destination transaction was submitted (PR-013G).
- [x] Guarded Solana -> Base live submit precheck reran approval on Render for destination hash `0x33c44d710e08d02ebd15492219ec1fd6a15682b69440351c850af017750df93b`; Base read-only checks still passed, but final `acceptBridgeMint` simulation failed with `DeadlineExpired`, so the submit command was not run and no Base destination transaction was submitted (PR-013H).
- [x] Fresh Solana -> Base source event generated from Codespace after Render's 2 GB web service exceeded memory during proof generation; paper replay and approval passed, final Base simulation passed, exactly one guarded Base `acceptBridgeMint` transaction `0x72b972a211e4950d110798523f6522b402dea83306f6e12805259bdd8adec983` was submitted for destination hash `0x67804661cc1d5fe7c0a54cc1c572a8c990d5ef5137580898d2c58f5b8e3c6865`, the tx confirmed, the destination hash is consumed, and duplicate submit blocks before a second send (PR-013I).
- [x] Base destination withdraw/recovery preparation added for the PR-013I Solana -> Base commitment. Read-only validation identifies destination commitment `0x12888fed12c64e6d6eebd6eb6c1859feb2ca45bc64319301ba9cdc6d562feef2`, Base preflight confirms tx `0x72b972a211e4950d110798523f6522b402dea83306f6e12805259bdd8adec983` is confirmed, consumed, and inserted, but withdraw preparation is blocked because the exact destination note-state with destination secret/nullifier was not found; no withdraw proof, simulation, or tx was attempted (PR-013J).
- [x] Final metadata-only recovery search did not recover exact PR-013I destination note-state, so the commitment is currently unrecoverable for withdrawal unless the exact note-state is later restored. Future Solana -> Base guarded live submit now has a hard durable Base destination note-state backup gate requiring validated, read-back note-state under `BRIDGE_BASE_NOTE_STATE_BACKUP_DIR` before `acceptBridgeMint` can be sent (PR-013K).
- [x] Solana -> Base submit-approved check-only mode added to exercise the durable Base destination note-state backup gate without sending. Missing backup now blocks before `writeContract`, valid fixture backup reaches `check_ready`, and the no-submit path keeps paper mode and live submit disabled (PR-013L).
- [x] Fresh Solana -> Base source fixture generated from Codespace with durable outside-repo Base destination note-state backup, paper replay reached `paper_ready_to_submit`, note-state validation/readback passed, approval simulation passed with gas estimate `969049`, and guarded submit check-only returned `check_ready` for destination hash `0xc204c9e91bc6c6e98e2fe25b6a3475cd32efc0da84b8e9017a96947bfad3c67d` without submitting a Base transaction (PR-013M).
- [x] Guarded one-shot Solana -> Base live submit completed for the PR-013M approved destination hash `0xc204c9e91bc6c6e98e2fe25b6a3475cd32efc0da84b8e9017a96947bfad3c67d`. Final check-only and the Base destination note-state gate passed, exactly one Base `acceptBridgeMint` tx `0x18b0d4a25ea9087630b0eed09d2399a33d16c8788290cad2d379619aedc96556` confirmed in block `41794491` using `957439` gas, the message is consumed, the commitment is inserted, duplicate submit is blocked, and live submit remained command-scoped with paper mode restored afterward (PR-013N).
- [x] Base destination withdraw preparation for the PR-013N commitment validated the durable Base note-state, confirmed the submit tx and inserted commitment, derived destination leaf index `42` from the submit-block `nextLeafIndex` delta, confirmed the destination nullifier is unspent and vault balance is sufficient, and blocked withdraw proof readiness on missing Base Merkle path/indexer evidence; no withdraw proof, simulation, or tx was attempted (PR-013O).
- [x] Base destination Merkle path recovery completed for PR-013N leaf `42`. Non-secret path evidence was exported outside git, path validation recomputed root `50015434963031949891316260787900094634376168319519755731383442155917094636`, withdraw proof generation succeeded in memory, read-only Base withdraw simulation passed with gas estimate `329772`, and no withdraw tx was submitted (PR-013P).
- [x] Guarded Base destination withdraw command added for PR-013Q. Final note-state validation, readback, Merkle path validation, Base preflight, nullifier, and vault checks passed, but the live withdraw failed closed before proof/send because no explicit `BRIDGE_WITHDRAW_RECIPIENT` or `BASE_WITHDRAW_RECIPIENT` is configured; no withdraw tx or extra `acceptBridgeMint` was submitted.
- [x] Explicit recipient gate tightened for PR-013R. The guarded Base withdraw rejects missing, invalid, or zero-address recipients before proof generation, simulation, or send; the current operator env still has no recipient configured, so no withdraw tx was submitted.
- [x] Guarded Base destination withdraw completed for PR-013S with reviewed recipient `0xC520f5545dc9Af65FF91470721Ee986e94a717d0`. Note-state and Merkle path validation passed, proof generation and simulation passed, withdraw tx `0x62e8047f599dacc5d4e8945336d5f134e3e3e438cd2a5f5119b545995ffe0095` confirmed in block `41835063`, the emitted nullifier is spent, recipient balance increased, vault balance decreased, and duplicate withdraw blocks before a second send.
...
- [x] Dependency audit triage, CI no-secret/artifact scan, `.gitignore` artifact hardening, and shared production rate-limit design completed. No dependency versions were changed because non-forced audit remediation is blocked by peer conflicts and force fixes are unsafe without dedicated compatibility testing (PR-014E).
