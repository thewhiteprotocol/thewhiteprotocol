...
Mainnet remains blocked. The current system has meaningful testnet controls and PR-014D added low-risk API hardening for public beta readiness. PR-014E adds CI no-secret/artifact scanning and a shared rate-limit design. PR-014F triages the scanner baseline and adds scanner tests, but production readiness still requires audit, custody, baseline cleanup/credential rotation, shared limiter implementation, dependency remediation, note-state custody, zkey ceremony, monitoring, and incident response.
...
| Repo structure / `.gitignore` | Hardened for future artifacts | Root `.gitignore` covers env files, note-state, keypairs, zkeys, data dirs, witnesses, proof JSON, and generated tx JSON; CI scanner is in `.github/workflows/security.yml`; PR-014F adds scanner unit tests and explicit baseline triage metadata | Historical public circuit artifacts and 19 pre-existing baseline findings are allowlisted by exact path/issue; future generated artifacts fail CI | Keep artifact scan required in CI, remove baseline findings, rotate active credentials, and review allowlisted public artifacts before mainnet |
...
| Secure env and secret management | Missing for production | `render.yaml` uses placeholders/comments for secrets; docs state env-file signers are testnet-only; PR-014F records 18 RPC-key indicators requiring rotation if active | Mainnet secret manager, rotation, and access logging are not formalized | Define secret classes, storage, rotation, incident process, and baseline cleanup |
...
| CI/CD and environment parity | Partially implemented and hardened | Existing workflows run tests; Render bootstrap documents zkey repair and fail-closed startup; `.github/workflows/security.yml` runs scanner self-test, scanner unit tests, no-secret/artifact scan, and non-gating audit | Dependency audit is non-gating until tracked findings are remediated or accepted; no-secret baseline must still be retired | Add SBOM, hosted parity checks, remove the no-secret baseline, and eventually make dependency policy gating |
...
- PR-014F: no-secret baseline cleanup and CI gate hardening completed; baseline remediation and credential rotation remain.
- PR-014G: production signer custody design.
- PR-014H: incident response and watcher/freeze production policy.
- PR-014I: circuit binding decision record.
